...
class NarreDateset(Dataset):
    def __init__(self, data: DataFrame,
                 ...
                 max_count: int, review_length: int):
    ...
    def load_reviews(self, review: Dict[str, DataFrame], query_id: str, exclude_id: str) -> List[List[int]]:
        ...
        3. Pad review text to max_count
        ...
        if len(reviews) >= self.max_count:
            reviews = reviews[:self.max_count]
        else:
            reviews += [[PAD_WORD_ID] * self.review_length] * (self.max_count - len(reviews))
        return reviews
